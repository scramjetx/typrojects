...
//RX buffer
const int BUFF_SIZE = 100;		//multiples of 5 for the buffer size cause message 5 bytes long, #lf50 ; header:cmd:dutyCycle	
...
while (1)
{
	...
	while(!ready)
	{
		...
		//WHILE freezes up the processor once alot of messages. IF doesn't seem to parse anything.
		if(RXcount > 0)		
			getCMD();		
